...
def _build_prompt(config: dict, state: str, mounting: Optional[str], extra_options: dict) -> str:
    ...
    slat_constraint = (
        f"      **CRITICAL CONSTRAINT: SLAT WIDTH — EXACTLY {_slat_w}**\n"
        f"      Each individual slat is exactly {_slat_w} wide.\n"
...
def _render_beta(img_bytes: bytes, mime: str, prompt: str) -> str:
    ...
    short_prompt = (
        ...
        "CRITICAL: The blind MUST be FULLY DEPLOYED — bottom rail rests ON the windowsill, "
        "covering the COMPLETE window height from top to bottom. "
        "NEVER show partially raised blinds. NEVER show the bottom rail floating above the sill. "
        + prompt[:1500]
    )
