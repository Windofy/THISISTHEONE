...
exact slat count, slat width, ladder tapes, ladder cords, tilt angle and color on a
transparent RGBA canvas. No AI involved here — every pixel is calculated.

Output is a flat front-on render.
"""
...
def render_blind_panel(
    ...
    Args:
      width_px, height_px : output canvas size
      config              : {colorName, colorHex, productType}
      state               : "Geheel uitgerold" (closed slats) or "Kantelen" (tilted slats)
      extra               : {slatWidth: "25mm"|"50mm", ladderType: "ladder_tape"|"ladder_cord"|"no_ladder",
                             lighting: ...}
    ...
    # Ladder type — mutually exclusive, mirrors LADDER_ALIASES in render_gemini.py
    ladder_type = extra.get("ladderType", "ladder_cord")
    has_tape    = ladder_type == "ladder_tape"
    ...
    if state == "Geheel uitgerold":
        ...
        warnings.warn(
            f"Unknown state '{state}', falling back to closed slats.",
            stacklevel=2,
        )
        _draw_closed_slats(draw, width_px, slat_top, slat_bot, slat_w_mm, px_per_mm, rgb)
    ...
    if has_tape:
        ...
        # Thin ladder cords only
        cord_color = _darken(rgb, 0.60)
        cord_w     = max(1, int(2 * px_per_mm))  # ~2 mm cord diameter
        for xf in tape_xs:
            x = int(width_px * xf)
            draw.line(
                [(x, slat_top), (x, slat_bot)],
                fill=(*cord_color, 220),
                width=cord_w,
            )
    # else: no_ladder — nothing drawn

    # ── Headrail (drawn over the top of slats area) ──
    ...
    # ── Lift cord (hangs from bottom rail, stays within canvas) ──
    if has_cords:
        lift_x       = int(width_px * 0.75)   # typically off-center to the right
        lift_len     = int(height_px * 0.25)
        lift_y_start = slat_bot
        lift_y_end   = min(height_px - 1, slat_bot + lift_len)
        lift_color   = _darken(rgb, 0.55)
        lift_w       = max(1, int(1.5 * px_per_mm))
        draw.line(
            [(lift_x, lift_y_start), (lift_x, lift_y_end)],
            fill=(*lift_color, 200),
            width=lift_w,
        )
...
def _draw_tilted_slats(draw, width_px, y_top, y_bot, slat_w_mm, px_per_mm, rgb,
    ...
    pitch_px     = slat_w_mm * px_per_mm                       # full slat-to-slat spacing
...
def _darken(rgb, factor):
    assert factor <= 1.0, f"darken factor must be <= 1.0, got {factor}"
    return tuple(max(0, min(255, int(c * factor))) for c in rgb)


def _lighten(rgb, factor):
    assert factor >= 1.0, f"lighten factor must be >= 1.0, got {factor}"
    return tuple(max(0, min(255, int(c * factor))) for c in rgb)
